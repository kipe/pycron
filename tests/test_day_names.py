...
class DayNamesTestCase(unittest.TestCase):
    ...
    def test_day_matching(self):
        def run(now):
            for i in range(0, 7):
                ...
                # Test weekdays
                assert pycron.is_now("* * * * mon,tue,wed,thu,fri", now) is (
                    True if i not in [0, 6] else False
                )
                assert pycron.is_now(
                    "* * * * monday,tuesday,wednesday,thursday,friday", now
                ) is (True if i not in [0, 6] else False)
                assert pycron.is_now("* * * * mon-fri", now) is (
                    True if i not in [0, 6] else False
                )
                assert pycron.is_now("* * * * monday-friday", now) is (
                    True if i not in [0, 6] else False
                )
                assert pycron.is_now("* * * * mon,tue,wed,thu-fri", now) is (
                    True if i not in [0, 6] else False
                ...
                # Test weekends
                assert pycron.is_now("* * * * sun,sat", now) is (
                    True if i in [0, 6] else False
                )
                assert pycron.is_now("* * * * sunday,saturday", now) is (
                    True if i in [0, 6] else False
                )
